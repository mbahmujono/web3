...
async function main () {
  ...
  // Mint a CowNFT
  const tokenURI = 'ipfs://your-token-uri' // Replace with actual token URI
  const amountOfTokensToMint = 1 // Replace with actual amount of tokens to mint

  const requestId = await cowFactory.sendRequestToMintCow(tokenURI, amountOfTokensToMint)
  console.log('Mint request sent with requestId:', requestId)
}
